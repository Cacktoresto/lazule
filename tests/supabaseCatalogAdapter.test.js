...
import catalogRepository, { getAllProducts, getCatalogProductsFromRepository, getCatalogSource, shouldUseSupabaseCatalog } from '../src/data/catalogRepository.js';
...
test('catalog repository exposes getAllProducts as named and default export compatibly', async () => {
  assert.equal(typeof getAllProducts, 'function');
  assert.equal(typeof catalogRepository, 'function');
  assert.equal(catalogRepository.getAllProducts, getAllProducts);
  assert.equal(catalogRepository.getCatalogProducts, getAllProducts);

  const products = await getAllProducts({ env: { VITE_CATALOG_SOURCE: 'local' } });
  assert.equal(products.length, getLocalCatalogProducts().length);
});
