...
function getRecommendationScore(currentProduct, candidate) {
  ...
  if (currentProduct.normalizedCategory && currentProduct.normalizedCategory === candidate.normalizedCategory) {
    score += 8;
  }
  ...
  if (currentProduct.normalizedGender && currentProduct.normalizedGender === candidate.normalizedGender) {
    score += 2;
  }
...
export function getProductRecommendations(currentProduct, allProducts = getCatalogProducts(), { min = 4, max = 8 } = {}) {
  ...
  const fallbackProducts = allProducts
    .filter((candidate) => candidate.id !== currentProduct.id && !rankedProducts.some((product) => product.id === candidate.id))
    .sort((a, b) => Number(b.featured) - Number(a.featured) || a.name.localeCompare(b.name, 'pt-BR'));

  return [...rankedProducts, ...fallbackProducts].slice(0, Math.max(min, Math.min(max, allProducts.length - 1)));
}
