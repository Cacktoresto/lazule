...
function AssistantResultCard({ recommendation, result, sourcePage }) {
  ...
  return (
    <article className="grid min-w-[17.5rem] snap-start grid-cols-[5.5rem_1fr] gap-4 overflow-hidden rounded-[1.5rem] border border-white/10 bg-lazule-night/70 p-3 shadow-mineral backdrop-blur sm:min-w-0">
      <a
      ...
      <div className="min-w-0 py-1">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <a
            className="rounded-full border border-emerald-300/30 bg-emerald-400/10 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-emerald-100 transition hover:bg-emerald-300 hover:text-lazule-night"
            href={whatsappLink}
            target="_blank"
            rel="noreferrer"
...
export function OlfactiveAssistant({ products = [], sourcePage = 'home', className = '' }) {
  ...
  return (
    <section className={`mx-auto max-w-7xl px-4 py-8 sm:px-8 sm:py-10 ${className}`} aria-labelledby="olfactive-assistant-title">
      <div className="relative overflow-hidden rounded-[2.2rem] border border-lazule-gold/20 bg-[radial-gradient(circle_at_18%_0%,rgba(200,162,77,0.18),transparent_28%),linear-gradient(135deg,rgba(15,23,42,0.92),rgba(30,58,138,0.58)_48%,rgba(5,8,22,0.94))] p-4 shadow-mineral sm:p-6 lg:p-8">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[0.82fr_1fr] lg:items-start">
          <div>
            ...
            <form className="mt-6 space-y-4" onSubmit={handleSubmit}>
              ...
              <textarea
                id="olfactive-query"
                className="min-h-28 w-full resize-none rounded-[1.45rem] border border-white/12 bg-lazule-night/70 px-4 py-4 text-base leading-6 text-lazule-mist outline-none transition placeholder:text-slate-500 focus:border-lazule-gold/70 focus:ring-2 focus:ring-lazule-gold/30"
                value={query}
                maxLength={180}
              ...
              <div className="lazule-horizontal-rail flex snap-x gap-2 overflow-x-auto pb-1 sm:flex-wrap sm:overflow-visible" aria-label="Sugestões rápidas">
                {QUICK_SUGGESTIONS.map((suggestion) => (
                  <button
                    key={suggestion}
                    type="button"
                    className="shrink-0 snap-start rounded-full border border-lazule-gold/25 bg-lazule-gold/10 px-3.5 py-2 text-sm font-semibold text-lazule-gold transition hover:border-lazule-gold/70 hover:bg-lazule-gold hover:text-lazule-night focus:outline-none focus:ring-2 focus:ring-lazule-gold"
          ...
          <div className="relative min-w-0 rounded-[1.8rem] border border-white/10 bg-white/[0.045] p-4 sm:p-5">
            {!result && !isLoading ? (
              ...
              <div>
                <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <h3 className="mt-2 font-display text-3xl leading-tight text-lazule-mist">{hasRecommendations ? 'Sua curadoria LAZULE' : 'Curadoria em ajuste'}</h3>
                  </div>
                ...
                {result.fallbackUsed ? (
                  <p className="mb-4 rounded-2xl border border-lazule-gold/20 bg-lazule-gold/10 px-4 py-3 text-sm leading-6 text-slate-200">
                    Não encontramos uma combinação perfeita, mas separamos opções versáteis.
                  </p>
                ) : null}

                <div className="lazule-horizontal-rail lazule-rail-fade flex snap-x gap-4 overflow-x-auto pb-2 lg:grid lg:grid-cols-2 lg:overflow-visible">
