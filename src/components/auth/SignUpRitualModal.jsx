...
export function SignUpRitualModal({ onClose, identitySeedFactory }) {
  const { signUpWithEmailPassword, isLoading, isAuthAvailable } = useAuth();
  ...
  const submit = async () => {
    ...
    const { error } = await signUpWithEmailPassword(email.trim(), password, metadata);

    if (error) {
      setMessage('Não conseguimos iniciar sua identidade agora.');
      setStep(4);
      return;
    }
