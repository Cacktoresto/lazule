  ...
  return (
    <form className={`w-full ${className}`} role="search" aria-label="Buscar e abrir perfume" onSubmit={handleSubmit}>
      <label className="block w-full" htmlFor="product-navigation-search">
        ...
        <div className="grid gap-3 rounded-[1.8rem] border border-lazule-gold/25 bg-lazule-night/75 p-2 shadow-aureate backdrop-blur sm:grid-cols-[1fr_auto] sm:rounded-full sm:p-2.5">
          <input
            id="product-navigation-search"
            className="min-h-12 rounded-[1.25rem] border border-white/10 bg-white/[0.08] px-4 py-3 text-base text-lazule-mist outline-none placeholder:text-slate-400 transition duration-200 hover:border-lazule-gold/40 focus-visible:border-lazule-gold/75 focus-visible:bg-white/[0.12] focus-visible:ring-2 focus-visible:ring-lazule-gold focus-visible:ring-offset-2 focus-visible:ring-offset-lazule-night sm:rounded-full"
            type="search"
            value={searchTerm}
            onChange={(event) => setSearchTerm(event.target.value)}
            placeholder="Digite Asad, Sauvage, Hacivat..."
            autoComplete="off"
            inputMode="search"
          />
          <button
            className="lazule-premium-button lazule-cta-shimmer min-h-12 rounded-full bg-lazule-gold px-6 text-sm font-semibold text-lazule-night shadow-aureate"
            type="submit"
          >
