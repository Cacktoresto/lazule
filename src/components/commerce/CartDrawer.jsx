...
const backdropClass = [
  'fixed inset-0 bg-[rgba(2,6,18,0.72)] transition duration-300',
  'backdrop-blur-[14px] backdrop-saturate-[120%]',
].join(' ');
const drawerSurfaceClass = [
  'fixed right-0 top-0 flex h-screen w-screen max-w-full flex-col overflow-hidden border-l border-lazule-gold/[0.18] max-sm:bottom-0 max-sm:top-auto max-sm:h-[min(92dvh,46rem)] max-sm:rounded-t-[1.65rem] sm:rounded-l-[2.35rem]',
  ...
  'transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] sm:w-[min(460px,100vw)]',
].join(' ');
const productCardClass = [
  ...
  'p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.075),0_18px_48px_rgba(0,0,0,0.30)] transition duration-300 hover:border-lazule-gold/30 sm:p-4',
].join(' ');
const footerClass = [
  'sticky bottom-0 z-20 shrink-0 border-t border-white/[0.1] pt-3 sm:pt-4',
  'bg-[linear-gradient(180deg,rgba(5,10,25,0),rgba(5,10,25,0.94)_18%,rgba(5,10,25,1))]',
].join(' ');
const drawerViewportContract = { height: '100dvh' };
...
export function CartDrawer({ open, onClose, items = [], total }) {
  ...
  return createPortal(
    <div className={`${drawerRootClass} ${open ? 'pointer-events-auto' : ''}`} aria-hidden={!open}>
      <button
        type="button"
      ...
      />
      <aside
        className={`${drawerSurfaceClass} ${open ? 'translate-x-0 max-sm:translate-y-0' : 'translate-x-full max-sm:translate-y-full'}`}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="relative z-10 flex h-full min-h-0 flex-col">
          <div className="sticky top-0 z-20 shrink-0 border-b border-white/[0.075] bg-[linear-gradient(180deg,rgba(3,7,19,0.99),rgba(3,7,19,0.94))] pb-3 pt-1 backdrop-blur-md sm:pb-4">
            <div className="flex items-start justify-between gap-5">
              <div className="min-w-0">
                <p className="text-[0.65rem] font-semibold uppercase tracking-[0.36em] text-lazule-gold/85">Seleção LAZULE</p>
                <h2 id="cart-drawer-title" className="mt-1 font-display text-2xl leading-tight text-white sm:mt-1.5 sm:text-3xl">Sua seleção</h2>
                <p className="mt-1 max-w-[20rem] text-xs leading-5 text-lazule-mist/68 sm:mt-2 sm:text-sm sm:leading-6">Curadoria concierge para revisar fragrâncias, quantidades e seguir para pagamento protegido.</p>
              </div>
              <button
                ref={closeButtonRef}
                type="button"
                onClick={onClose}
                className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/[0.12] bg-white/[0.065] text-lg leading-none text-lazule-mist/75 transition hover:border-lazule-gold/45 hover:bg-lazule-gold/12 hover:text-lazule-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lazule-gold/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#030713]"
                aria-label="Fechar seleção"
              >
                ×
              </button>
          ...
          <div className="min-h-0 flex-1 space-y-2.5 overflow-y-auto py-2.5 pr-1 sm:space-y-3 sm:py-3 [scrollbar-width:thin] [scrollbar-color:rgba(200,162,77,0.45)_transparent]">
            {hasItems ? items.map((item) => <article key={item.id} className={productCardClass}>
              ...
              <div className="relative z-10 flex gap-4">
                {item.image ? (
                  <img src={item.image} alt={item.name} className="h-[64px] w-[64px] shrink-0 sm:h-[78px] sm:w-[78px] rounded-[1.2rem] bg-slate-950 object-cover shadow-[0_14px_34px_rgba(0,0,0,0.42)]" />
                ) : (
                  <div className="flex h-[64px] w-[64px] shrink-0 sm:h-[78px] sm:w-[78px] items-center justify-center rounded-[1.2rem] border border-lazule-gold/18 bg-slate-950/80 text-[0.65rem] font-semibold tracking-[0.28em] text-lazule-gold/60">LAZ</div>
                )}
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 line-clamp-1 text-xs leading-5 text-lazule-mist/68 sm:mt-2 sm:line-clamp-2">{item.editorialPhrase}</p>
                  <div className="mt-3 flex items-center justify-between gap-3">
                    <label className="flex items-center gap-2 rounded-full border border-white/[0.1] bg-black/25 px-2.5 py-1.5 text-[0.65rem] uppercase tracking-[0.18em] text-lazule-mist/55">
                      Qtd.
                      <input aria-label={`Quantidade de ${item.name}`} type="number" min="1" value={item.quantity} onChange={(event) => upsertLuxuryQuantity(item.id, event.target.value)} className="w-10 bg-transparent text-center text-sm font-semibold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lazule-gold/60 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" />
                    </label>
                    <button type="button" onClick={() => removeFromLuxurySelection(item.id)} className="min-h-10 rounded-full px-3 py-2 text-xs font-medium text-lazule-mist/55 transition hover:text-lazule-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lazule-gold/60">Remover</button>
                  </div>
                </div>
              </div>
            </article>) : (
              <div className="flex min-h-[13rem] flex-col sm:min-h-[18rem] items-center justify-center rounded-[1.75rem] border border-white/[0.08] bg-white/[0.035] px-8 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.055)]">
          ...
          <div className={footerClass}>
            <div className="flex items-end justify-between gap-4">
              ...
              <p className="font-display text-2xl leading-none text-white sm:text-3xl">{formatBRL(total)}</p>
            </div>
            {hasItems ? (
              <>
                <button type="button" onClick={handleCheckoutClick} disabled={isCheckingOut} className="lazule-premium-button lazule-cta-shimmer mt-3 block w-full rounded-full border border-[#d8bb72]/65 bg-[linear-gradient(135deg,#f1d991_0%,#c8a24d_42%,#8f6b2f_100%)] px-5 py-3.5 text-center text-sm font-bold uppercase tracking-[0.16em] sm:py-4 sm:tracking-[0.2em] text-[#07101f] shadow-[0_18px_48px_rgba(200,162,77,0.24),inset_0_1px_0_rgba(255,255,255,0.42)] transition hover:shadow-[0_22px_58px_rgba(200,162,77,0.32),inset_0_1px_0_rgba(255,255,255,0.50)] disabled:cursor-wait disabled:opacity-75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-lazule-gold/80 focus-visible:ring-offset-2 focus-visible:ring-offset-[#030713]">{isCheckingOut ? 'Iniciando pagamento...' : 'Finalizar compra'}</button>
              ...
              <>
                <button type="button" disabled className='mt-4 block w-full cursor-not-allowed rounded-full border border-white/[0.12] bg-white/[0.055] px-5 py-3.5 text-center text-sm font-bold uppercase tracking-[0.16em] sm:py-4 sm:tracking-[0.2em] text-lazule-mist/45'>Finalizar compra</button>
