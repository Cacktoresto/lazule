...
const drawerSurfaceClass = [
  'absolute right-0 top-0 h-full w-full overflow-hidden border-l border-white/[0.06]',
  'bg-[rgba(5,10,25,0.98)] text-lazule-mist shadow-[-24px_0_80px_rgba(0,0,0,0.45)]',
  'backdrop-blur-md transition duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] sm:w-[30rem] sm:rounded-l-[2rem]',
].join(' ');
const productCardClass = [
  'group relative overflow-hidden rounded-[1.35rem] border border-white/[0.07]',
  'bg-[linear-gradient(145deg,rgba(255,255,255,0.055),rgba(255,255,255,0.018)_42%,rgba(200,162,77,0.045))]',
  'p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.055),0_18px_48px_rgba(0,0,0,0.24)]',
].join(' ');
        ...
        <div className="relative z-10 flex h-full flex-col">
          <div className="flex items-start justify-between gap-5 pb-5">
            <div className="min-w-0">
              <p className="text-[0.65rem] font-semibold uppercase tracking-[0.36em] text-lazule-gold/80">Curadoria LAZULE</p>
              <h2 className='mt-1.5 font-display text-2xl leading-tight text-white'>Sua seleção</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className='min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 [scrollbar-width:thin] [scrollbar-color:rgba(200,162,77,0.35)_transparent]'>
            {items.length ? items.map((item) => <article key={item.id} className={productCardClass}>
              ...
              <div className='relative z-10 flex gap-4'>
                {item.image ? (
                  <img src={item.image} alt={item.name} className='h-[72px] w-[72px] shrink-0 rounded-2xl bg-slate-950 object-cover shadow-[0_12px_30px_rgba(0,0,0,0.36)]'/>
                ) : (
                  <div className='flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-2xl border border-white/[0.07] bg-slate-950/80 text-[0.65rem] font-semibold tracking-[0.28em] text-lazule-gold/60'>LAZ</div>
                )}
                <div className='min-w-0 flex-1'>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <p className='shrink-0 text-sm font-semibold text-white'>{formatBRL(item.price || 0)}</p>
                  </div>
          ...
          <div className={totalPanelClass}>
            <div className="flex items-end justify-between gap-4">
              <p className='text-[0.68rem] font-semibold uppercase tracking-[0.28em] text-lazule-mist/48'>Total</p>
              <p className='font-display text-3xl leading-none text-white'>{formatBRL(total)}</p>
            </div>
            <a href='/checkout' className='lazule-premium-button lazule-cta-shimmer mt-5 block w-full rounded-full border border-[#d8bb72]/55 bg-[linear-gradient(135deg,#f1d991_0%,#c8a24d_42%,#8f6b2f_100%)] px-5 py-4 text-center text-sm font-bold uppercase tracking-[0.2em] text-[#07101f] shadow-[0_18px_48px_rgba(200,162,77,0.20),inset_0_1px_0_rgba(255,255,255,0.38)] transition hover:shadow-[0_22px_58px_rgba(200,162,77,0.28),inset_0_1px_0_rgba(255,255,255,0.46)]'>Finalizar compra</a>
          </div>
        </div>
      </aside>
    </div>
