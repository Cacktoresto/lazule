...
export function InfluencerInvite({ token }) {
  ...
  if (isLoading) {
    return <InviteState title="Validando seu convite" description="Estamos conferindo seu acesso exclusivo ao programa de parceiros LAZULE." />;
  }
  ...
  if (!isPartnerInviteAcceptable(invite)) {
    return (
      <InviteState
        ...
        action={<a className="inline-flex rounded-full border border-lazule-gold/40 px-5 py-3 text-sm font-semibold text-lazule-gold transition hover:bg-lazule-gold/10" href="/influencer/login">Ir para login</a>}
      />
    );
  }
  ...
  return (
    <section className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8 lg:py-16">
      <aside className="relative overflow-hidden rounded-[2.5rem] border border-lazule-gold/20 bg-gradient-to-br from-lazule-blue/35 via-slate-950 to-lazule-night p-8 shadow-2xl shadow-lazule-blue/20 lg:p-10">
        ...
        <h1 className="mt-6 text-4xl font-semibold leading-tight text-white lg:text-5xl">Você foi convidado para o programa de parceiros LAZULE.</h1>
        <p className="mt-5 text-sm leading-7 text-lazule-mist/72">Seu acesso é privado, rastreável e liberado apenas por convite. Finalize a senha para abrir seu painel, links de indicação e acompanhamento de performance.</p>
      ...
      <div className="rounded-[2rem] border border-lazule-gold/20 bg-slate-950/78 p-6 shadow-2xl shadow-lazule-blue/20 backdrop-blur sm:p-8 lg:p-10">
        <p className="text-xs font-semibold uppercase tracking-[0.34em] text-lazule-gold/80">Onboarding premium</p>
        <h2 className="mt-4 text-3xl font-semibold text-white">Crie sua senha</h2>
        <p className="mt-3 text-sm leading-6 text-lazule-mist/68">Usaremos o e-mail do convite e a role influencer. Não há cadastro público nem escolha de permissões administrativas.</p>
        <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
          <label className="block text-sm font-medium text-lazule-mist/86">Senha
            <input className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-white outline-none transition placeholder:text-lazule-mist/38 focus:border-lazule-gold/60 focus:ring-2 focus:ring-lazule-gold/20" type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} disabled={isSubmitting} />
          </label>
          <label className="block text-sm font-medium text-lazule-mist/86">Confirmar senha
            <input className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-white outline-none transition placeholder:text-lazule-mist/38 focus:border-lazule-gold/60 focus:ring-2 focus:ring-lazule-gold/20" type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} disabled={isSubmitting} />
          </label>
          ...
          <button className="w-full rounded-full bg-lazule-gold px-6 py-3 text-sm font-bold uppercase tracking-[0.22em] text-lazule-night shadow-lg shadow-lazule-gold/20 transition hover:-translate-y-0.5 hover:bg-[#f5d77a] disabled:cursor-not-allowed disabled:opacity-55 disabled:hover:translate-y-0" type="submit" disabled={isSubmitting}>{isSubmitting ? 'Finalizando...' : 'Finalizar cadastro'}</button>
        </form>
      </div>
    </section>
  );
