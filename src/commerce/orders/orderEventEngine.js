...
export function createEventFingerprint({ orderId, paymentId, type, source, payload = {} } = {}) {
  return createHash(stableStringify({ orderId, paymentId, type, source, payload }));
}

export function createOrderEvent({ orderId, type, source = 'system', payload = {}, paymentId = null, fingerprint = null, timestamp = null } = {}) {
...
export function appendOrderEvent(order, eventInput) {
  const current = order || {};
...
export function listOrderEvents(order) {
